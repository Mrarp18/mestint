...
class QLearningAgent:
    ...
    # We use the epsilon-greedy policy to choose the next action
    ...
    # We update the Q-table using the Q-learning algorithm
...
rewards, epsilon_history = play_episodes(
    n_episodes=50_000, max_epsilon=1.0, min_epsilon=0.05, decay_rate=0.0001, gamma=0.95, learn=True, viz=False,
...
rewards, epsilon_history = play_episodes(10, max_epsilon=0, min_epsilon=0, decay_rate=0, gamma=0, learn=False, viz=True,
                                         log=True)
plt.plot(epsilon_history)
plt.show()
plt.plot(rewards)
